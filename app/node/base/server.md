...
http.createServer((req, res) => {
    ...
    if (pathname !== '/favicon.ico') { // 过滤请求
        fs.readFile(`static${pathname}`, (err, data) => {
            ...
            // 返回这个文件
            const mime = mimeModel.getMime(fs, extname) // 获取文件类型
            res.writeHead(200, {
...
function getMime(callback = data => data) {
    fs.readFile('mime.json', (err, data) => {
        if (err) {
            console.log(err)
            return
...
const mimeModel = require('./model/getmimefromfile_callback')
...
+ getmimefromfile_callback.js

```js
exports.getMime = (fs, extname, callback = result => result) => {
    fs.readFile('./mime.json', (err, data) => {
        if (err) {
            console.log('mime.json文件不存在')
            return
...
http.createServer((req, res) => {
    ...
    if (pathname !== '/favicon.ico') { // 过滤请求
        fs.readFile(`static${pathname}`, (err, data) => {
            ...
            EventEmitter.once('to_mime', mime => {
                res.writeHead(200, {
                    'Content-Type': `${mime};charset="utf-8"`
                })
                // res.write(data)
...
> EventEmitter.on 程序包内存泄露,可以使用EventEmitter.once
